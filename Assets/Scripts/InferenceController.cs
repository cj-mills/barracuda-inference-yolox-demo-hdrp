...
public class InferenceController : MonoBehaviour
{
    ...
    private void Awake()
    {
        inferenceManager.inferenceController = this;
    }
    ...
    /// </summary>
    /// <param name="inputDims">The input dimensions for processing</param>
    private void UpdateBoundingBoxes(Vector2Int inputDims, Vector2 screenDims)
    {
    ...
    public void Infernece(RenderTexture imageTexture)
    {

        if (!Application.isPlaying) return;

        //Debug.Log($"W:{imageTexture.width} H:{imageTexture.height}");
        ...
        Vector2 screenDims = new Vector2(imageTexture.width, imageTexture.height);

        // Update bounding boxes and user interface
        UpdateBoundingBoxes(inputDims, screenDims);
